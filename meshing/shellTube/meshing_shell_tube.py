...
if len(actual_volume_tags) != 3:
    ...
    print(
        "You may need to inspect the geometry in gmsh GUI to identify correct volume tags"
    )
...
if len(actual_volume_tags) >= 3:
    gmsh.model.addPhysicalGroup(
        3, [actual_volume_tags[0]], walls_vol_marker, name="walls"
    )
    gmsh.model.addPhysicalGroup(
        3, [actual_volume_tags[1]], coolant_vol_marker, name="coolant"
    )
    gmsh.model.addPhysicalGroup(
        3, [actual_volume_tags[2]], breeder_vol_marker, name="breeder"
    )
else:
...
gmsh.model.addPhysicalGroup(
    2,
    walls_breeder_interfaces,
    walls_breeder_interfaces_marker,
    name="walls_breeder_interfaces",
)
gmsh.model.addPhysicalGroup(
    2,
    walls_coolant_interfaces,
    walls_coolant_interfaces_marker,
    name="walls_coolant_interfaces",
)
